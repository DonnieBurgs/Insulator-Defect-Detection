...
def assign_anchor(feat_shape, gt_boxes, im_info, feat_stride=16,
    ...
    inds_inside = np.where((all_anchors[:, 0] >= -50) & # -allowed_border) &
                           (all_anchors[:, 1] >= -50) & # -allowed_border) &
                           (all_anchors[:, 2] < im_info[1] + 50) & # + allowed_border) &
                           (all_anchors[:, 3] < im_info[0] + 50))[0] # + allowed_border))[0]
    if len(inds_inside) == 0:
        print "inds_inside is: ", inds_inside
        print "case1: ", np.where((all_anchors[:, 0] >= -allowed_border))[0]
        print "case2: ", np.where((all_anchors[:, 0] >= -allowed_border) &
                           (all_anchors[:, 1] >= -allowed_border))[0]
        print "case3: ", np.where((all_anchors[:, 0] >= -allowed_border) &
                           (all_anchors[:, 1] >= -allowed_border) &
                           (all_anchors[:, 2] < im_info[1] + allowed_border))[0]
        print "im_info is: ", im_info

        tmp_case3 = np.where((all_anchors[:, 0] >= -allowed_border) &
                           (all_anchors[:, 1] >= -allowed_border) &
                           (all_anchors[:, 2] < im_info[1] + allowed_border))[0]
        for i in tmp_case3:
            print "all_anchors[%d] is: "%(i), all_anchors[i]
    ...
    # print "anchors = all_anchors[inds_inside, :]:  ", anchors
    ...
    if gt_boxes.size > 0:
        ...
        # overlaps (ex, gt)
        if len(anchors) == 0:
	    print "anchors:", anchors, "gt_boxes: ", gt_boxes
